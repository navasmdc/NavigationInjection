...
open abstract class BaseActivity : AppCompatActivity(), NavigationManager {
    ...
    private fun switchState(state: BaseState) {
        ...
        nextOperation.onStateChanged(currentState, state, {
            if (!javaClass.equals(nextOperation.Activity().java) || !state.Activity().java.equals(Default::class.java)) {
                val intent =
                        if (!state.Activity().java.equals(Default::class.java))
                            Intent(this, state.Activity().java)
                        else
                            Intent(this, nextOperation.Activity().java)
